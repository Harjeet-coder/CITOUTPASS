...
export default function HODDashboard() {
  ...
  const handleApprove = (id) => {
    updateRequestStatus(id, 'Approved by HOD', 'Security');
    alert(`Outpass request ${id} approved. Sent to Security.`);
  };

  const handleReject = (id) => {
    updateRequestStatus(id, 'Rejected by HOD', null);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {requests.filter(req => req.status === 'Approved by Teacher').length === 0 ? (
        <Text style={styles.noRequestsText}>No requests pending for HOD approval.</Text>
      ) : (
        requests
          .filter(req => req.status === 'Approved by Teacher')
          .map((request) => (
