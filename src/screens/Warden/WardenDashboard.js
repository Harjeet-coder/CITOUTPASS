...
export default function WardenDashboard() {
  ...
  const handleApprove = (id) => {
    updateRequestStatus(id, 'Approved by Warden', 'Security');
    alert(`Outpass request ${id} approved. Sent to Security.`);
  };

  const handleReject = (id) => {
    updateRequestStatus(id, 'Rejected by Warden', null);
    alert(`Outpass request ${id} rejected. Student will be notified.`);
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {requests.filter(req => req.status === 'Approved by HOD').length === 0 ? (
        <Text style={styles.noRequestsText}>No requests pending for Warden approval.</Text>
      ) : (
        requests
          .filter(req => req.status === 'Approved by HOD')
          .map((request) => (
